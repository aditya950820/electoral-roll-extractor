...
# ---------------------------------------------------------------- compare PDF
# For the model rules (fuzzy_new / cosine_new) each flag carries a full
# per-attribute `comparison` — the logic that produced it. This report prints
# that logic beside the two voters, so blocks are tall and variable in height:
# it flows them down the page and starts a new page when the next block would
# not fit, capping at COMPARE_PER_PAGE.
COMPARE_PER_PAGE = 5
...
def _draw_compare_block(page, f, y: float, M: float, pw: float,
    ...
    # comparison table: Attribute | Voter A | Voter B | Match. Cells use
    # insert_text (baseline point) with width-based truncation — insert_textbox
    # silently drops single lines when the row is shorter than the line height.
    if comp:
        total = pw - 2 * M
...
def build_flags_pdf_zip(rule_filter: str | None, year: int | None = None,
    ...
    counterpart of the single combined report. `builder` picks the per-AC PDF
    function (defaults to build_flags_pdf; the model rules pass build_compare_pdf)."""
    import io
    import zipfile
